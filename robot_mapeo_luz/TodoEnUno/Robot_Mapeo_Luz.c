...
const int distanciaAvanzar=(3333);
...
void ConfiguraUART_HC05(void){ //***************CONFIGURAR UART1********************//
	...
	UART5_IBRD_R = (UART5_IBRD_R & ~UART_IBRD_DIVINT_M)|104; // IBRD =int(16,000,000/(16*960000)) = int(104.16667)
	UART5_FBRD_R = (UART5_FBRD_R & ~UART_FBRD_DIVFRAC_M)|10; // FBRD = round(0.166667 * 64)= 10.666668
...
uint32_t GirarIzquierda(int frecuencia,int dutycycle,int TiempoDeGiro){
	...
	tiempo=OndaPWM_DERECHO_TIMER1(frecuencia+3,dutycycle);  // Movemos Motor derecho
...
uint32_t GirarDerecha(int frecuencia,int dutycycle,int TiempoDeGiro){
	...
	tiempo=OndaPWM_DERECHO_TIMER3(frecuencia+3, dutycycle);  // Retrocedemos Motor Derecho
	retardo_ms(TiempoDeGiro);         // Cercano a 90°
	DetenerTodo();            // Detenemos cualquier movimiento
	return tiempo;
}
uint32_t Avanzar(int frecuencia,int dutycycle,int TiempoDeGiro){
	...
	DetenerTodo();
	tiempo=OndaPWM_IZQUIERDO_TIMER1(frecuencia,dutycycle);  // Movemos Motor Izquierdo
	tiempo=OndaPWM_DERECHO_TIMER1(frecuencia+3,dutycycle);  // Movemos Motor Izquierdo
	retardo_ms(TiempoDeGiro);         // Cercano a 90°
	DetenerTodo();
	return tiempo;
}
uint32_t Retroceder(int frecuencia,int dutycycle,int TiempoDeGiro){
	...
	tiempo=OndaPWM_DERECHO_TIMER3(frecuencia+3, dutycycle);  // Movemos Motor Izquierdo
...
void main (void){
	...
	uint8_t PrimeraTecla=0;         	    // Teclas que almacenarán las letras
	uint32_t tiempo=0;
	...
	while(1)
	{
		...
		TeclasPresionadas=PrimeraTecla; // Recibimos Primera Tecla
		...
		case 0x64:  // Girar Derecha dd
			...
			angulo-=15;                                             	// Angulo aparente que realiza cada vez que gira
			break;
		default:
		...
		else
		{
			...
			txmens_uart_HC05(guion);	//Si es negativo pone un guion adelante
		...
		}else
		{
			...
			txmens_uart_HC05(guion);	//Si es negativo pone guion adelante
